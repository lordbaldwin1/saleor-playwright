...
Playwright will run `saleor:up:detached` and `storefront:dev` before tests. Locally it is usually easier to keep Docker and `pnpm dev` running in separate terminals.
...
4. Start storefront (`pnpm dev`)
5. Run **all** Playwright projects (`api`, `dashboard`, `storefront`)
